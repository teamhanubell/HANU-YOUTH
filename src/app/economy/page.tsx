...
export default function EconomyPage() {
  useEffect(() => {
    // Award small bonus for visiting the economy page
    const awardVisitBonus = async () => {
      try {
        await fetch('/api/gamification/economy', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            currency_type: 'coins',
            source: 'page_visit',
            description: 'Visited economy page'
          })
        })
      } catch (error) {
        console.error('Error awarding visit bonus:', error)
      }
    }
        ...
        <header className="text-center mb-12 reveal-on-scroll">
          <div className="mb-6">
            ...
            <p className="text-lg md:text-xl text-slate-600 mb-2">
              Earn coins, gems, and rewards for your learning achievements
            </p>
            <p className="text-base text-slate-600">
              Shop for power-ups, unlock features, and trade with the global community.
            </p>
          </div>
        </header>

        {/* Virtual Economy Component */}
